...
Here's the minimal setup to draw a five-pointed star:
...
If you’re building your own shape or working with dynamic vertices, you can use:

```rust
use graph1::draw::polygons::closed_perimeter;
closed_perimeter(ctx, &vertices, Some(color));
```

This draws a connected loop (a closed polyline) between all points. It’s used internally by `polygon()` and `star()`.

---

## 🖌️ Flood Fill

You can fill an enclosed area using `draw::tools::fill::flood`. It’s a scanline-based flood-fill algorithm:

```rust
use graph1::draw::tools::fill::flood;

let seed = Pixel { x: 100, y: 100, color: RetroNeon::GLITCH_RED };
flood(ctx, &seed);
```

This can be useful for game editors or interactive drawing tools.
...
All gradients preserve the alpha channel — so you can smoothly fade transparency along with color.<br />
